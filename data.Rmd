...
# RR 계산
dt$E_e9 <- dt$population_total * (sum(dt$e9) / sum(dt$population_total))
dt$RR <- dt$e9 / dt$E_e9
...
```
```{r}
plot_sgg_var(dt_q, sgg, var = e9)
plot_sgg_var(dt_q, sgg, var = e9_1)
plot_sgg_var(dt_q, sgg, var = e9_2)
plot_sgg_var(dt_q, sgg, var = RR)

```
```{r}
top10_regions <- dt_y %>%
  filter(yyyymm == 202112) %>%           # 2025년 6월 데이터만 필터링
  arrange(desc(RR)) %>%                  # RR 내림차순 정렬 (높은 순)
  select(canon_sigungu_name, RR, e9, E_e9) %>% # 지역명, RR, 관측값, 기대값 확인
  head(10)                               # 상위 10개 추출
...
ggplot(subset_data, aes(x = time_id, y = RR, group = sgg_code10)) +
  ...
  # (3) 축 및 테마 설정
  scale_x_continuous(
    breaks = time_labels$time_id[seq(1, nrow(time_labels), by = 2)], # 라벨이 너무 많으면 2칸씩 띄움
    labels = time_labels$yq[seq(1, nrow(time_labels), by = 2)]
  ) +
  ...
  labs(
    title = "Relative Risk (RR) Trends of 100 Randomly Selected Regions",
