...
- 아래 두 파일의 output은 업로드되어 있습니다.
    ...
    - (`data.Rmd`: 패널 데이터(`panel_full_*.csv`) 이용해서 `dt_q_mod.csv`, `dt_y_mod.csv` 생성)'
- `model.Rmd`: 베이지안 계층 모형 적합

2. 발표 자료, 분석 계획서, 중간 분석 결과 보고서

3. 기타 자료
- `sgis_sgg_merged/SGG_MERGED.gpkg`: 시군구 경계 파일
- `panel_full_*.csv`: 시군구 단위 패널 데이터 (종속변수 + 설명변수)
-  `dt_q_variable_dictionary.xlsx`: `dt_q_mod.csv` 변수 설명

## `_data` (note: these files are not necessarily updated)
1. `.ipynb` 코드 파일
