...
fit_baseline_q <- inla(
  e9_f ~ 1 + offset(log(population_total)) + f(time_id, model = "ar1",# scale.model = TRUE,
             hyper = list(prec = list(prior = "pc.prec", param = c(0.8, 0.5)))),
  data = dt_q,
...
```{r}
dt_y <- dt_y %>%
  mutate(sex_ratio = as.numeric(scale(population_male / population_female)))
```
...
fit_cov_y <- inla(
  ...
  data    = dt_y,
  family  = "poisson",
  E       = dt_y$population_total,   # ★ 인구를 기대노출로
  control.compute   = list(dic=TRUE, waic=TRUE, cpo=TRUE),
  control.predictor = list(link=1, compute=TRUE),
  control.inla      = list(strategy="simplified.laplace",
                           int.strategy = "auto",
